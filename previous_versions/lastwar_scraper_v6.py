"""
lastwar_scraper_v6.py
---------------------
Automate collection of the Top 10 players for Groups A–P from
Last War: Survival (Windows client) and export to Excel.

Key features:
- One-time calibration of UI coords (F1–F7).
- OCR-based dropdown click (no fixed Y offsets).
- Drag (or wheel) scrolling with image-diff “ensure_top/bottom” (no drift).
- Robust parsing: alliance = text inside [ ], player = text right after ],
  warzone and kill score found flexibly.
- Hard failsafe: press ESC or Ctrl+C anytime to quit.
- Rich debug dumps (--debug) to inspect OCR results.

Usage:
  python lastwar_scraper_v6.py --calibrate
  python lastwar_scraper_v6.py --run leaderboard.xlsx --debug
"""
...
signal.signal(signal.SIGINT, _stop_handler)   # Ctrl+C
try:
    signal.signal(signal.SIGBREAK, _stop_handler)  # Windows Ctrl+Break
except Exception:
    pass
...
DEFAULT_CONFIG = {
    ...
    "tesseract_exe": "",
    "lang": "eng",                      # add "eng+kor+jpn" if needed
    ...
    "max_scroll_attempts_top": 12,
    "max_scroll_attempts_bottom": 8,
...
def bring_window_to_front(rx: str):
    ...
    if not titles:
        print("[WARN] Game window not found.")
        return
...
def grab(region: Tuple[int, int, int, int]) -> Image.Image:
    with mss.mss() as sct:
        s = sct.grab({"left": region[0], "top": region[1],
...
def ocr_tsv(img: Image.Image, psm: int, lang: str, exe: str) -> pd.DataFrame:
    ...
    df = pytesseract.image_to_data(img, config=f"--psm {psm}",
                                   lang=lang, output_type=Output.DATAFRAME)
...
def calibrate(cfg: Dict):
    ...
    # Selecting a group will close it automatically later.
    ...
    save_config(cfg)
    print("Saved config.json. Run:\n  python lastwar_scraper_v6.py --run --debug")
...
def ocr_click_group(cfg: Dict, letter: str):
    ...
    if target_xy is None:
        print(f"[WARN] Couldn't OCR 'Group {letter}'. Using fallback math.")
...
def _grab_strip(cfg: Dict, where="top", height=90) -> np.ndarray:
...
def ensure_top(cfg: Dict):
    ...
    for _ in range(cfg["max_scroll_attempts_top"]):
        check_abort()
        strip = _grab_strip(cfg, "top")
        if prev is not None and np.array_equal(strip, prev):
            break
        # scroll up once
        if cfg["scroll_method"] == "wheel":
            ...
            scroll_drag(tuple(cfg["ui"]["drag_to"]), tuple(
                cfg["ui"]["drag_from"]), 1, cfg["sleep_after_scroll"])
        prev = strip
    ...
    for _ in range(cfg["max_scroll_attempts_bottom"]):
        ...
        if cfg["scroll_method"] == "wheel":
            ...
            scroll_drag(tuple(cfg["ui"]["drag_from"]), tuple(
                cfg["ui"]["drag_to"]), 1, cfg["sleep_after_scroll"])
        prev = strip
...
def df_to_clusters(df: pd.DataFrame) -> List[pd.DataFrame]:
    """Cluster words by mid_y proximity to form visual rows."""
    if df.empty:
        return []
    df = df.copy()
...
def parse_row_tokens(tokens: List[str]) -> Dict:
    text = " ".join(tokens)
    if "Group " in text:     # ignore dropdown junk
        return None
...
def scrape_group(cfg: Dict, gname: str) -> List[Dict]:
    # Reset to top and capture
    ensure_top(cfg)
    img_top = capture_board(cfg)

    # Go to bottom and capture
    ensure_bottom(cfg)
...
if __name__ == "__main__":
    ...
    print("Usage:")
    print("  python lastwar_scraper_v6.py --calibrate")
    print("  python lastwar_scraper_v6.py --run [outfile.xlsx] [--debug]")
